...
colors = plt.cm.inferno_r(sp.linspace(0.2, 0.5, 2))
#colors = plt.cm.viridis_r(sp.linspace(0.5, 0.8, 2))
...
for iM in range(obj.Mm)[:num_receps]:
	plt.plot(dSss, vals[:, iM], color='k', lw=lw, alpha=0.8)
...
for iSs0, mu_Ss0 in enumerate(mu_Ss0s):
	...
	for iS, dSs in enumerate(dSss):
		...
		vals[iS, :] = obj.Yy
	for iM in range(obj.Mm)[:num_receps]:
		plt.plot(dSss, vals[:, iM], color=colors[iSs0], lw=lw, alpha=0.8)
