...
def plot_subfigures(data_flag):
	...
	# Cycle colors a bit to give a more aesthetic feel
	colors = cm.Greys(sp.linspace(0.6, 0.50, num_figs))
	...
	for fig_num, iM in enumerate(iMs_to_plot):
		...
		plt.plot(sp.arange(params['Nn']/2 - 1, params['Nn'] - 1), RHS_curve, 
							color=color, lw=lw)
		
		save_tuning_curve_fig(fig, mu_dSs_idx, sigma_Kk2_idx, fig_num, data_flag)
